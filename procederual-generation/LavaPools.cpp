...
void LavaPools::Lava(std::vector<std::vector<char>>& map, int lava_pits, int seed) {
    ...
    for(int i=0;i<lava_pits;i++){
        ...
        for(int j = 0;j<lava_pit_size;j++){
            ...
            if(dy+y_coordinate<height and dx+x_coordinate<width and dy+y_coordinate>=0 and dx+x_coordinate>=0) map[dy+y_coordinate][dx+x_coordinate] = 'L';
            if(dy+y_coordinate<height and dx+x_coordinate+1<width and dy+y_coordinate>=0 and dx+x_coordinate+1>=0) map[dy+y_coordinate][dx+x_coordinate+1] = 'L';
            if(dy+y_coordinate<height and dx+x_coordinate-1<width and dy+y_coordinate>=0 and dx+x_coordinate-1>=0) map[dy+y_coordinate][dx+x_coordinate-1] = 'L';
            if(dy+y_coordinate+1<height and dx+x_coordinate<width and dy+y_coordinate+1>=0 and dx+x_coordinate>=0) map[dy+y_coordinate+1][dx+x_coordinate] = 'L';
            if(dy+y_coordinate-1<height and dx+x_coordinate<width and dy+y_coordinate-1>=0 and dx+x_coordinate>=0) map[dy+y_coordinate-1][dx+x_coordinate] = 'L';
        }
    }
}
