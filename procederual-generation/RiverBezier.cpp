    ...
    for(int i{0};i<presision;i++){
        ...
        if(bridge_amount!=0 and i%(presision/bridge_amount)==(presision/(bridge_amount*2))){
            ...
            for(int j=0;j<presision/30;j++){
                ...
                x_result = (std::pow(1-t, 3)*control_points[first_id].getX())+(3*std::pow(1-t, 2)*(t)*control_points[second_id].getX())+(3*std::pow((t), 2)*(1-t)*control_points[third_id].getX())+(std::pow(t,3)*control_points[fourth_id].getX());
                y_result = std::pow(1-t, 3)*control_points[first_id].getY()+(3*std::pow(1-t, 2)*(t)*control_points[second_id].getY())+(3*std::pow(t, 2)*(1-t)*control_points[third_id].getY())+(std::pow(t,3)*control_points[fourth_id].getY());
                Point a(std::round(x_result), std::round(y_result));
                x_result = (std::pow(1-t, 3)*control_points[first_id].getX())+(3*std::pow(1-t, 2)*t*control_points[second_id].getX()+1)+(3*std::pow(t, 2)*(1-t)*control_points[third_id].getX())+(std::pow(t,3)*control_points[fourth_id].getX());
                y_result = std::pow(1-t, 3)*control_points[first_id].getY()+(3*std::pow(1-t, 2)*t*control_points[second_id].getY()+0.5)+(3*std::pow(t, 2)*(1-t)*control_points[third_id].getY())+(std::pow(t,3)*control_points[fourth_id].getY());
