    ...
    switch (in_action) {
        case (0): {
            if (in_state->getCoordinates().first == height) return in_state;
                ...
                return coordinates_to_state[std::make_pair(in_state->getCoordinates().first,
                                                           in_state->getCoordinates().second + 1)];
            }
        }
        case (1): {
            if (in_state->getCoordinates().first == width) return in_state;
            else {
                return coordinates_to_state[std::make_pair(in_state->getCoordinates().first + 1,
                                                           in_state->getCoordinates().second)];
            }
        }
        case (2): {
