...
public:
    ...
    Environment(int height, int width, int seed, bool deterministic, float living_reward, float end_reward,
                float percentage_obstacles);
...
private:
    ...
    void insert_non_deterministic_dynamics(MDPState *current_state, int action, float reward, float probability,
                                           MDPState *next_state);
};
